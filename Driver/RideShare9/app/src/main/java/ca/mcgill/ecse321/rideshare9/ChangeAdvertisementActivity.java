...
public class ChangeAdvertisementActivity extends AppCompatActivity {
                    ...
                    public void onClick(DialogInterface dialog,int which) {
                        ...
                        HttpUtils.post(getApplicationContext(), "stop/add-stop",headers, entity, "application/json", new JsonHttpResponseHandler(){
                            ...
                            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                                ...
                                advertisement.getStops().add(s);

                                stopListView = findViewById(R.id.stop_list);
                                stopListView.setAdapter(new StopList_adapter(advertisement.getStops(), getApplicationContext()));
    ...
    public void modifyJourney(View v){
        ...
        if(startLocation.trim().equals("") || tripTitle.trim().equals("")){
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        startLocationView.setText(advertisement.getStartLocation());

        // end location
        TextView endLocationView = (TextView) findViewById(R.id.endLocationText);
        endLocationView.setText(advertisement.getTitle());

        TextView tripTitle  = (TextView) findViewById(R.id.trip_title_text);
        tripTitle.setText(advertisement.getTitle());

        String dateTime[] = advertisement.getStartTime().split(" ");
        // start time
        TextView startTimeView = (TextView) findViewById(R.id.starttime);
        startTimeView.setText(dateTime[1]);


        // date
        TextView startDateView = (TextView) findViewById(R.id.startDate);
        startDateView.setText(dateTime[0]);
        //set up stop list
        stopListView = findViewById(R.id.stop_list);
        stopListView.setAdapter(new StopList_adapter(advertisement.getStops(), getApplicationContext()));
